...
var SectionSummaryDesc = Desc{
	"subscription": "Subscriptions defined here will be resolved as nodes and merged as a part of the global node pool.\nSupport to give the subscription a tag, and filter nodes from a given subscription in the group section.\nPlain Clash YAML subscriptions are detected automatically and import Shadowsocks, SOCKS5, HTTP(S), VMess, VLESS, Trojan, Hysteria2, TUIC, AnyTLS, and Snell proxies. oixcloud:// and oixcloud+file:// fetch, verify, and decrypt managed Clash YAML.",
...
var DnsDesc = Desc{
	...
	"upstream":         "Value can be scheme://host:port, where the scheme can be udp, tcp, tcp+udp, tls, https, quic, h3, or http3. Prefix a remote scheme with oixcloud+ to enable oixCloud query authentication, for example oixcloud+udp://host:53.\nIf host is a domain and has both IPv4 and IPv6 record, dae will automatically choose IPv4 or IPv6 to use according to group policy (such as min latency policy).\nPlease make sure DNS traffic will go through and be forwarded by dae, which is REQUIRED for domain routing.\nIf dial_mode is \"ip\", the upstream DNS answer SHOULD NOT be polluted, so domestic public DNS is not recommended.",
