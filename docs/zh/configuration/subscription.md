...
dae 原有的 HTTP(S)、本地文件、SIP008 和 Base64 订阅行为保持不变。构建产物嵌入 `OIXCLOUD_SUBSCRIPTION_HMAC_KEY` 后，还可以读取 oixCloud 托管配置。
...
dae 会认证请求、生成临时 age X25519 身份、校验服务端提供的响应签名、解密 YAML，然后读取根级 `proxies`。当前仅转换 `type: anytls` 和 `type: snell`；其他代理类型会跳过并输出汇总告警。Clash 策略组、规则、DNS 和 provider 不会被导入。
